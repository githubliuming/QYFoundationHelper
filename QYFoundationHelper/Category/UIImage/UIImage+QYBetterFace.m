...
{
    NSArray *features = [UIImage _qy_faceFeaturesInImage:self accuracy:accurary];
    ...
    else
    {
        ...
        return [self _qy_subImageForFaceFeatures:features size:size];
    }
}

- (UIImage *)_qy_subImageForFaceFeatures:(NSArray *)faceFeatures size:(CGSize)size
{
    ...
    CGPoint fixedCenter =
        CGPointMake(fixedRect.origin.x + fixedRect.size.width / 2.0, fixedRect.origin.y + fixedRect.size.height / 2.0);
...
+ (NSArray *)_qy_faceFeaturesInImage:(UIImage *)image accuracy:(QYAccuracy)accurary
{
    CIImage *ciImage = [CIImage imageWithCGImage:image.CGImage];
    NSString *accuraryStr = (accurary == QYAccuracyLow) ? CIDetectorAccuracyLow : CIDetectorAccuracyHigh;

    CIDetector *detector =
        [CIDetector detectorOfType:CIDetectorTypeFace context:nil options:@{CIDetectorAccuracy : accuraryStr}];
