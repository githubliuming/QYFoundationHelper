...
+ (UIImage *)fixOrientation:(UIImage *)srcImg
{
    ...
    CGContextRef ctx =
        CGBitmapContextCreate(NULL, srcImg.size.width, srcImg.size.height, CGImageGetBitsPerComponent(srcImg.CGImage),
                              0, CGImageGetColorSpace(srcImg.CGImage), CGImageGetBitmapInfo(srcImg.CGImage));
 ...
 *  @return  翻转后的图片
 */
- (UIImage *)flipVertical { return [self flip:NO]; }
/**
 *  @brief  水平翻转
 *
 *  @return 翻转后的图片
 */
- (UIImage *)flipHorizontal { return [self flip:YES]; }
/**
 *  @brief  旋转图片
 *
 *  @param degrees 弧度
