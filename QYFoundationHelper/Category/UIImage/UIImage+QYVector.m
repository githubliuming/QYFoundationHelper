...
#import "UIImage+QYVector.h"
#import <CoreText/CoreText.h>
@implementation UIImage (QYVector)
...
{
    NSString *identifier = [NSString stringWithFormat:@"%@%@%@%@%d%f", NSStringFromSelector(_cmd), font.fontName,
                                                      tintColor, iconNamed, clipToBounds, fontSize];
...
+ (UIImage *)imageWithPDFFile:(NSString *)pdfFile withTintColor:(UIColor *)tintColor forSize:(CGSize)size
{
    ...
    NSString *identifier = [NSString
        stringWithFormat:@"%@%@%@%@", NSStringFromSelector(_cmd), pdfFile, tintColor, NSStringFromCGSize(size)];
    UIImage *image = [[self cache] objectForKey:identifier];
    if (image)
    {
        return image;
